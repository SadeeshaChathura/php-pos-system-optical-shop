...
class Goodreceive extends CI_Controller {
	...
	public function Goodreceivestatus($idtbl_grn, $status) {
		$porder_id = $this->input->get('porder_id');
	
		$this->load->model('Goodreceiveinfo');
		$result = $this->Goodreceiveinfo->Goodreceivestatus($idtbl_grn, $status, $porder_id);
	}
	...
	public function Getlastgrnprice(){
		$this->load->model('Goodreceiveinfo');
        $result=$this->Goodreceiveinfo->Getlastgrnprice();
	}

	/**
	 * Streams the GRN as a printable A4 PDF, opened in a new tab.
	 * Uses a separate model (Goodreceiveprintinfo) so the print/PDF
	 * logic stays independent from the CRUD/listing model, matching
	 * the same pattern used for Purchase Order printing.
	 */
	public function Getgrnprintpdf($x){
		$this->load->model('Goodreceiveprintinfo');
		$this->Goodreceiveprintinfo->Getgrnprintpdf($x);
	}
}