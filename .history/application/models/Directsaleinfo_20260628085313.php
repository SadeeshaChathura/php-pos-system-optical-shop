...
class Directsaleinfo extends CI_Model {
    ...
    public function Directsaleinsertupdate(){
        ...
        foreach ($tableData as $rowtabledata) {
            ...
            // Deplete stock FIFO across batches, same logic as before
        ...
        if ($billtype == 1 || $billtype == 2 || (!empty($tableDataPay) && is_array($tableDataPay) && count($tableDataPay) > 0)) {
            ...
            if (!empty($tableDataPay)) {
                foreach ($tableDataPay as $rowPay) {
                    // col_1 method label, col_2 bank/card-type, col_3 branch, col_4 chequeno/last4, col_5 chequedate, col_7 amount
                    $dataone = array(
                        'method'        => $rowPay['col_1'],
                        'amount'        => $rowPay['col_7'],
                        'bank'          => $rowPay['col_2'],   // re-used for card type when method = Card
                        'branch'        => $rowPay['col_3'],
                        'chequeno'      => $rowPay['col_4'],   // re-used for last-4-digits when method = Card
                        'chequedate'    => $rowPay['col_5'],
                        'status'        => 1,
                        'insertdatetime'=> $insertdatetime,
                        'tbl_user_idtbl_user' => $userID,
