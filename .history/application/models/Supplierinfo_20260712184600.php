...
class Supplierinfo extends CI_Model{
    ...
    public function Getmaterialinfo(){
        $this->db->select('`idtbl_material_info`, `materialinfocode`, `materialname`');
        $this->db->from('tbl_material_info');
        $this->db->where('status', 1);

        return $respond=$this->db->get();
    }
    public function Getmaterialinfobycategory(){
        ...
        if(!empty($categoryID)){
            $this->db->where_in('tbl_material_category_idtbl_material_category', $categoryID);
        }
        ...
        foreach($respond->result() as $row){
            $obj = new stdClass();
            $obj->id   = $row->idtbl_material_info;
            $obj->text = $row->materialname . '-' . $row->materialinfocode;
            array_push($materialinfoarray, $obj);
        }

        echo json_encode($materialinfoarray);
    }
    public function generateSupplierCode(){
        // Prefix is "S" (1 char), so the numeric part starts at position 2
        $this->db->select("MAX(CAST(SUBSTRING(suppliercode, 2) AS UNSIGNED)) as maxcode", false);
    ...
    public function Supplierinsertupdate(){
        ...
        $code=$this->input->post('suppliercode');
