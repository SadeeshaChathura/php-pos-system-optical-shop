...
class Directsalecreditreceiptinfo extends CI_Model {
    ...
    public function Getcreditprintbill($x){
        ...
        ========================== */
        $extraTotalsHtml = '
            <div class="paid-row"><span>' . ($ordertype == 2 ? 'Total Paid' : 'Advance Paid') . '</span><span>Rs. ' . number_format($totalpay, 2) . '</span></div>
        ';
        if (!$isSettled) {
            $extraTotalsHtml .= '<div class="due-row"><span>Balance Due</span><span>Rs. ' . number_format($balancedue, 2) . '</span></div>';
        }

        /* =========================
           STATUS PILL — now actually used (this was the bug)
        ========================== */
        if ($isSettled) {
            $statusPillCls  = '';
            ...
            if ($ordertype == 2) {
                ...
                $noticeHtml = '
                    <div class="credit-notice">
                        &#9888; <span><b>Advance Order:</b> Balance of <b>Rs. ' . number_format($balancedue, 2) . '</b> is due on collection' . $dueTxt . '.</span>
                    </div>
                ';
            } else {
                $noticeHtml = '
