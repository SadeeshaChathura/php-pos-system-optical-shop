...
class Directsalereceiptinfo extends CI_Model {
    ...
    public function Getposprintbill($x){
        ...
        $invtype       = $invoiceinforespond->row(0)->invtype;     // 1 Cash, 1 Card, 3 Credit
        $warrantystatus= $invoiceinforespond->row(0)->warrantystatus;
        $paymentgiven  = $invoiceinforespond->row(0)->paymentgiven;
        $changegiven   = $invoiceinforespond->row(0)->changegiven;
        ...
        /*
        * Card type / last-4 are still stored via tbl_invoice_payment_detail
        * historically for Card sales display text (bank/chequeno). If you've
        * also stopped writing those for billtype 2, this block simply finds
        * nothing and the labels stay blank — safe either way.
        */
        $cardTypeLabel = '';
        $cardLast4     = '';

        if ($invtype == 2) {
            $sqlcard = "SELECT `bank`, `chequeno`
                        ...
                        LIMIT 1";
            $cardRespond = $this->db->query($sqlcard, array($recordID));
            if ($cardRespond->num_rows() > 0) {
                $cardTypeLabel = $cardRespond->row(0)->bank;
                $cardLast4     = $cardRespond->row(0)->chequeno;
            }
        }
        ...
        if ($invtype == 1) {
            $extraTotalsHtml = '
                <div class="paid-row"><span>Cash Received</span><span>Rs. ' . number_format($paymentgiven, 2) . '</span></div>
                <div class="change-row"><span>Change Given</span><span>Rs. ' . number_format($changegiven, 2) . '</span></div>
            ';
        } elseif ($invtype == 2) {
            $extraTotalsHtml = '
                <div class="paid-row"><span>Amount Charged</span><span>Rs. ' . number_format($paymentgiven, 2) . '</span></div>
            ';
        }
