...
class Directsalecreditreceiptinfo extends CI_Model {
    ...
    public function Getcreditprintbill($x){
        ...
        $statusPillText = ($balancedue <= 0) ? '&#10003; PAID IN FULL' : '&#8987; CREDIT &mdash; BALANCE DUE';
        $statusPillCls  = ($balancedue <= 0) ? '' : 'credit';

        $noticeHtml = ($balancedue <= 0)
            ? '' // no outstanding-balance warning once settled
            : '<div class="credit-notice">&#9888; <span><b>Important:</b> Please settle the outstanding balance of <b>Rs. '.number_format($balancedue,2).'</b>.</span></div>';
     ...
     * no dependency on the other and can be rendered standalone.
     */
