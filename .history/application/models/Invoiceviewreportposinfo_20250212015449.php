...
class Invoiceviewreportposinfo extends CI_Model{
    public function printreportpos($x){
		...
		$sqlinvoiceinfo="SELECT `tbl_invoice`.`idtbl_invoice`, `tbl_invoice`.`invdate`, `tbl_invoice`.`grosstotal`, `tbl_invoice`.`discount`, `tbl_invoice`.`nettotal`, `tbl_invoice`.`paycomplete` FROM `tbl_invoice` WHERE `tbl_invoice`.`status`=1 AND `tbl_invoice`.`idtbl_invoice`=?";
		...
		$sqlproduct="SELECT `tbl_material_info`.`materialinfocode`,`tbl_material_info`.`materialname`, `tbl_material_info`.`comment`, `tbl_invoice_detail`.`qty`, `tbl_invoice_detail`.`saleprice` FROM `tbl_invoice_detail` LEFT JOIN `tbl_material_info` ON `tbl_material_info`.`idtbl_material_info`=`tbl_invoice_detail`.`tbl_material_info_idtbl_material_info` WHERE `tbl_invoice_detail`.`tbl_invoice_idtbl_invoice`=? AND `tbl_invoice_detail`.`status`=1";
		...
		$sqlproductcount="SELECT COUNT(`tbl_material_info`.`materialinfocode`) AS itemcount, COUNT(`tbl_material_info`.`materialname`) AS materialname, `tbl_material_info`.`comment`, SUM(`tbl_invoice_detail`.`qty`) AS sumqty, `tbl_invoice_detail`.`saleprice` FROM `tbl_invoice_detail` LEFT JOIN `tbl_material_info` ON `tbl_material_info`.`idtbl_material_info`=`tbl_invoice_detail`.`tbl_material_info_idtbl_material_info` WHERE `tbl_invoice_detail`.`tbl_invoice_idtbl_invoice`=? AND `tbl_invoice_detail`.`status`=1";
		...
		$sqlpayment="SELECT SUM(`nettotal`) AS `sumpayment`, SUM(`balance`) AS `balanceamount` FROM `tbl_invoice_payment` LEFT JOIN `tbl_invoice_payment_has_tbl_invoice` ON `tbl_invoice_payment_has_tbl_invoice`.`tbl_invoice_payment_idtbl_invoice_payment`=`tbl_invoice_payment`.`idtbl_invoice_payment` WHERE `tbl_invoice_payment_has_tbl_invoice`.`tbl_invoice_idtbl_invoice`=?";
		...
		if (empty($paymentrespond)) {
			...
			$balance = $paymentrespond->row(0)->balanceamount;
		}
		...
		$sqlinvoicedetail="SELECT `tbl_material_info`.`materialinfocode`, `tbl_invoice_detail`.`qty`, `tbl_invoice_detail`.`saleprice` FROM `tbl_invoice_detail` LEFT JOIN `tbl_material_info` ON `tbl_material_info`.`idtbl_material_info`=`tbl_invoice_detail`.`tbl_material_info_idtbl_material_info` WHERE `tbl_invoice_detail`.`tbl_invoice_idtbl_invoice`='$recordID' AND `tbl_invoice_detail`.`status`=1";
	...
	<body>
		<div id="DivIdToPrint">
			<img src="'.base_url().'images/test_company.jpg" alt=""
			...
			<table style="width:100%;">
				...
				<tr>
					<td style="text-align: left; font-size:14px;font-weight: bold;">Balance</td>
				...
				<tr>
					<td style="text-align: center; font-size:10px;" colspan="2"><span
							style="text-align: center; font-size:16px;font-weight: bold;">Thank You. Come again!</span><br>Test Company(PVT)Ltd <br> 53, 3rd LANE, City, 011-2635185</td>
				</tr>
				<tr>
					<td style="text-align: center; font-size:5px;" colspan="2"><span
							style="text-align: center; font-size:8px;font-weight: bold;">Copyright © CalciteX IT Solutions</td>
				</tr>
			</table>
		</div>
