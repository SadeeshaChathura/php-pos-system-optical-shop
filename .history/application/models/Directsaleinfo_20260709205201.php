...
class Directsaleinfo extends CI_Model {
    ...
    public function Directsaleinsertupdate()
    {
        ...
        // PAYMENT INSERT
        // =========================
        if ($billtype == 1 || $billtype == 2 || $billtype == 4) {
            ...
            if (!empty($tableDataPay) && is_array($tableDataPay)) {
                ...
                foreach ($tableDataPay as $rowPay) {
                    ...
                    $paymentDetail = array(
                        'method' => ($billtype == 4) ? (isset($rowPay['col_1']) && $rowPay['col_1'] === 'Card' ? 2 : 1) : $billtype,
                        'amount' => $nettotal,
    ...
    /* =========================================================
 * ADVANCE / PRODUCTION ORDERS
 * ========================================================= */

public function Getpendingadvanceorders(){
    $searchTerm = $this->input->post('searchTerm');

    $sql = "SELECT i.idtbl_invoice, i.invdate, i.duedate, i.nettotal,
                   c.name AS customername, c.contact AS customercontact, c.customercode,
    ...
    if (!empty($searchTerm)) {
        if (is_numeric($searchTerm)) {
            $sql .= " AND (i.idtbl_invoice = ? OR c.name LIKE ? OR c.contact LIKE ?)";
            $params[] = $searchTerm;
            $params[] = '%'.$searchTerm.'%';
            $params[] = '%'.$searchTerm.'%';
        } else {
            $sql .= " AND (c.name LIKE ? OR c.contact LIKE ? OR c.customercode LIKE ?)";
            $params[] = '%'.$searchTerm.'%';
            $params[] = '%'.$searchTerm.'%';
            $params[] = '%'.$searchTerm.'%';
        }
    ...
    foreach ($result->result() as $r) {
        ...
        $list[] = array(
            'invoiceid'   => $r->idtbl_invoice,
            'invno'       => 'INV-' . str_pad($r->idtbl_invoice, 6, '0', STR_PAD_LEFT),
            ...
            'balance'     => $balance,
            'overdue'     => ($r->duedate && $r->duedate < date('Y-m-d')) ? 1 : 0,
        );
    }
...
public function Getadvanceorderbalance(){
    $invoiceID = $this->input->post('invoiceID');

    $sql = "SELECT i.idtbl_invoice, i.nettotal, i.duedate,
            ...
            WHERE i.idtbl_invoice = ?";

    $r = $this->db->query($sql, array($invoiceID))->row();

    $obj = new stdClass();
    if (!$r) { $obj->found = false; echo json_encode($obj); return; }

    $obj->found      = true;
    $obj->invoiceid  = $r->idtbl_invoice;
    $obj->customer   = $r->customername ?: 'Customer';
    $obj->contact    = $r->customercontact;
    $obj->duedate    = $r->duedate;
    $obj->nettotal   = (float)$r->nettotal;
    $obj->paidsofar  = (float)$r->paidsofar;
    $obj->balance    = (float)$r->nettotal - (float)$r->paidsofar;
...
public function Settleadvancebalance(){
    ...
    $invoice = $this->db->get_where('tbl_invoice', array('idtbl_invoice' => $invoiceID))->row();

    $obj = new stdClass();

    if (!$invoice || $invoice->ordertype != 2 || $invoice->paycomplete == 1) {
        $obj->success = false;
        $obj->message = 'This order is not a pending advance order.';
        echo json_encode($obj);
        return;
    }
    ...
    $this->db->insert('tbl_invoice_payment_detail', array(
        'method'         => $methodCode,
        'amount'         => $amount,
        'bank'           => ($methodCode == 2) ? $cardType : '',
        'branch'         => '',
        'chequeno'       => ($methodCode == 2) ? $cardLast4 : '',
    ...
    if ($paidSoFar >= $invoice->nettotal) {
        $updateData['paycomplete']   = 1;
        $updateData['completeddate'] = $insertdatetime;
    }
    ...
    if ($this->db->trans_status() === TRUE) {
        ...
        $this->db->trans_rollback();
        $obj->success = false;
        $obj->message = 'Could not record payment. Please try again.';
    }
