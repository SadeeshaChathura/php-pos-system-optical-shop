...
class Invoiceviewinfo extends CI_Model{
    ...
    public function Getproductlisttoselectpicker(){
        ...
        if(!isset($searchTerm)){
            $sql="SELECT `idtbl_material_info`, `productcode` FROM `tbl_material_info` WHERE `status`=? LIMIT 5";
            $respond=$this->db->query($sql, array(1));                       
        }
        else{            
            if(!empty($searchTerm)){
                $sql="SELECT `idtbl_material_info`, `productcode` FROM `tbl_material_info` WHERE `status`=? AND `productcode` LIKE '$searchTerm%'";
                $respond=$this->db->query($sql, array(1));    
            }
            else{
                $sql="SELECT `idtbl_material_info`, `productcode` FROM `tbl_material_info` WHERE `status`=? LIMIT 5";
