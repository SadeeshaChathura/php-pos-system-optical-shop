...
class Invoiceviewreportposinfo extends CI_Model{
    ...
    $sqlinvoiceinfo="SELECT `tbl_invoice`.`idtbl_invoice`, `tbl_invoice`.`invdate`, `tbl_invoice`.`grosstotal`, `tbl_invoice`.`discount`, `tbl_invoice`.`nettotal`, `tbl_invoice`.`paycomplete`, `tbl_customer`.`name`, `tbl_customer`.`address` FROM `tbl_invoice`  LEFT JOIN `tbl_customer` ON `tbl_customer`.`idtbl_customer`=`tbl_invoice`.`tbl_customer_idtbl_customer` WHERE `tbl_invoice`.`status`=1 AND `tbl_invoice`.`idtbl_invoice`=?";
    ...
    $sqlproductcount="SELECT COUNT(`tbl_material_info`.`materialname`) AS itemcount, `tbl_material_info`.`comment`, SUM(`tbl_invoice_detail`.`qty`) AS sumqty, `tbl_invoice_detail`.`saleprice` FROM `tbl_invoice_detail` LEFT JOIN `tbl_material_info` ON `tbl_material_info`.`idtbl_material_info`=`tbl_invoice_detail`.`tbl_material_info_idtbl_material_info` WHERE `tbl_invoice_detail`.`tbl_invoice_idtbl_invoice`=? AND `tbl_invoice_detail`.`status`=1";
	...
	foreach ($productrespond->result() as $rowlist) {
		$tblinvoice .= '
			...
			<td style="text-align: left; font-size:14px;" colspan="4">' .
					$rowlist->materialname . ' - ' . $rowlist->materialinfocode . '</td>
			</tr>
			<tr>
    ...
    $sqlinvoicedetail="SELECT `tbl_material_info`.`materialname`, `tbl_invoice_detail`.`qty`, `tbl_invoice_detail`.`saleprice` FROM `tbl_invoice_detail` LEFT JOIN `tbl_material_info` ON `tbl_material_info`.`idtbl_material_info`=`tbl_invoice_detail`.`tbl_material_info_idtbl_material_info` WHERE `tbl_invoice_detail`.`tbl_invoice_idtbl_invoice`='$recordID' AND `tbl_invoice_detail`.`status`=1";
