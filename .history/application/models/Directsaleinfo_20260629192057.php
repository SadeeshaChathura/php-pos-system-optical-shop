...
class Directsaleinfo extends CI_Model {
    ...
    public function Directsaleinsertupdate(){
        ...
        /*
        * paymentgiven / changegiven are ONLY meaningful for Cash (1) and Card (2) sales.
        * Cash: paymentgiven = amount tendered by customer, changegiven = paymentgiven - nettotal
        * Card: paymentgiven = amount charged (== nettotal), changegiven = 0
        * Credit (3): both stay 0 here — credit advance/balance is tracked via
        * tbl_invoice_payment / tbl_invoice_payment_detail as before.
        */
        $paymentgiven = 0;
        $changegiven  = 0;
...
/*
 * tbl_invoice_payment / tbl_invoice_payment_detail are used for Credit Sales.
 * Always create a payment header for a credit invoice, even if there is no
 * advance payment. Payment details are inserted only when available.
 */
if ($billtype == 3) {
    ...
    if (!empty($tableDataPay) && is_array($tableDataPay)) {
        ...
        foreach ($tableDataPay as $rowPay) {
            ...
            $paymentDetail = array(
                ...
                'amount'                                    => isset($rowPay['col_6']) ? $rowPay['col_6'] : 0,
