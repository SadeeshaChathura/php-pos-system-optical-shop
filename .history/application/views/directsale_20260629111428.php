...
function openBatchPicker(product){
    ...
    $.post(BASE_URL + 'Directsale/Getproductbatches', { productID: product.id }, function(result){
        ...
        var html = '<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(180px,1fr));gap:12px;padding:4px 2px;">';
        batches.forEach(function(b, idx){
            var dateStr   = b.date ? b.date.substring(0, 10) : '—';
            var stockCls  = b.qty <= 0 ? 'out' : (b.qty <= 5 ? 'low' : 'in');
            ...
            html +=
                ...
                ' onmouseover="this.style.borderColor=\'var(--primary)\';this.style.boxShadow=\'0 4px 18px rgba(0,0,0,0.10)\'"' +
                ' onmouseout="this.style.borderColor=\'var(--border)\';this.style.boxShadow=\'none\'">' +

                /* top accent bar */
                '<div style="position:absolute;top:0;left:0;right:0;height:4px;background:var(--primary);border-radius:14px 14px 0 0;"></div>' +

                /* batch number */
                '<div style="margin-top:4px;">' +
                  '<div style="font-size:10px;color:var(--muted2);text-transform:uppercase;letter-spacing:.7px;font-weight:600;">Batch No.</div>' +
                  '<div style="font-size:15px;font-weight:700;color:var(--primary-dark);margin-top:2px;">' + escapeHtml(b.batchno || 'N/A') + '</div>' +
                '</div>' +
                ...
                '<div style="display:flex;justify-content:space-between;align-items:center;">' +
                  '<div>' +
                    '<div style="font-size:10px;color:var(--muted2);text-transform:uppercase;letter-spacing:.7px;font-weight:600;">Sale Price</div>' +
                    '<div style="font-size:18px;font-weight:800;color:var(--success);margin-top:1px;">' + money(b.saleprice) + '</div>' +
                  '</div>' +
                  '<div style="text-align:right;">' +
                    '<div style="font-size:10px;color:var(--muted2);text-transform:uppercase;letter-spacing:.7px;font-weight:600;">Qty</div>' +
                    '<div style="font-size:18px;font-weight:800;color:var(--primary-dark);margin-top:1px;">' + b.qty + '</div>' +
                  '</div>' +
                '</div>' +

                /* stock badge + date */
                '<div style="display:flex;justify-content:space-between;align-items:center;">' +
                  '<span style="font-size:11px;font-weight:700;padding:3px 10px;border-radius:20px;background:' + stockBg + ';color:' + stockColor + ';">' + stockLbl + '</span>' +
                  '<span style="font-size:10.5px;color:var(--muted2);"><i class="fas fa-calendar-alt" style="margin-right:3px;font-size:9px;"></i>' + dateStr + '</span>' +
                '</div>' +

                /* select button */
                '<div style="background:var(--primary);color:#fff;border-radius:8px;padding:7px;text-align:center;font-size:12px;font-weight:700;letter-spacing:.3px;">' +
                  '<i class="fas fa-hand-pointer" style="margin-right:5px;"></i>Select This Batch' +
                '</div>' +
