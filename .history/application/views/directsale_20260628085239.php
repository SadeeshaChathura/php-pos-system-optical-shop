    ...
    <div id="layoutSidenav_content">
        <main>
            <div class="container-fluid p-0 pos-container">
                ...
                <div class="pos-app">
                    ...
                    <div class="pos-left">
                        <div class="pos-search-row">
                            ...
                            <button class="pos-btn-icon" id="btnPrevBills" title="Previous bills">
                                <i class="fas fa-receipt"></i>
                            </button>
                        </div>

                        <div class="pos-category-row" id="categoryRow">
                            <div class="pos-cat-chip active" data-cat=""><i class="fas fa-border-all"></i> All Items</div>
                    ...
                    <div class="pos-right">
                        ...
                        <div class="pos-cart-head">
                            <h3><i class="fas fa-cart-shopping"></i> Current Sale <span class="count" id="cartCount">0</span></h3>
                            <button class="pos-clear-btn" id="btnClearCart"><i class="fas fa-trash-can"></i> Clear</button>
                        </div>
                ...
                <!-- Hidden cart table, same convention as before, used purely as the data source for createinvoice() -->
...
<div class="pos-modal-overlay" id="modalCustomerSelect">
    <div class="pos-modal-box wide">
        <div class="pos-modal-header">
            ...
            <button onclick="closePosModal('modalCustomerSelect')"><i class="fas fa-xmark"></i></button>
        </div>
...
<div class="pos-modal-overlay" id="modalNewCustomer">
    <div class="pos-modal-box">
        <div class="pos-modal-header">
            ...
            <button onclick="closePosModal('modalNewCustomer')"><i class="fas fa-xmark"></i></button>
        </div>
        <div class="pos-modal-body">
...
<div class="pos-modal-overlay" id="modalAddToCart">
    <div class="pos-modal-box">
        <div class="pos-modal-header">
            ...
            <button onclick="closePosModal('modalAddToCart')"><i class="fas fa-xmark"></i></button>
        </div>
        <div class="pos-modal-body">
...
<div class="pos-modal-overlay" id="modalBrowse">
    <div class="pos-modal-box xwide">
        <div class="pos-modal-header">
            <h3><i class="fas fa-table-list"></i> Browse Products</h3>
            <button onclick="closePosModal('modalBrowse')"><i class="fas fa-xmark"></i></button>
        </div>
        <div class="pos-modal-body">
...
<div class="pos-modal-overlay" id="modalPayCash">
    <div class="pos-modal-box">
        <div class="pos-modal-header">
            ...
            <button onclick="closePosModal('modalPayCash')"><i class="fas fa-xmark"></i></button>
        </div>
        <div class="pos-modal-body">
...
<div class="pos-modal-overlay" id="modalPayCard">
    <div class="pos-modal-box">
        <div class="pos-modal-header">
            ...
            <button onclick="closePosModal('modalPayCard')"><i class="fas fa-xmark"></i></button>
        </div>
        <div class="pos-modal-body">
            <div class="pos-bill-summary-box">
...
<div class="pos-modal-overlay" id="modalCreditBlocked">
    <div class="pos-modal-box">
        <div class="pos-modal-header">
            ...
            <button onclick="closePosModal('modalCreditBlocked')"><i class="fas fa-xmark"></i></button>
        </div>
        <div class="pos-modal-body">
            <div style="text-align:center;padding:20px 10px;">
                <i class="fas fa-triangle-exclamation" style="font-size:34px;color:var(--warning);"></i>
        ...
        <div class="pos-modal-footer">
            ...
            <button class="pos-btn pos-btn-primary" id="btnCreditSelectCustomer"><i class="fas fa-user-check"></i> Select Customer</button>
        </div>
    </div>
</div>
...
<div class="pos-modal-overlay" id="modalCreditConfirm">
    <div class="pos-modal-box">
        <div class="pos-modal-header">
            ...
            <button onclick="closePosModal('modalCreditConfirm')"><i class="fas fa-xmark"></i></button>
        </div>
        <div class="pos-modal-body">
...
<div class="pos-modal-overlay" id="modalPrevBills">
    <div class="pos-modal-box xwide">
        <div class="pos-modal-header">
            ...
            <button onclick="closePosModal('modalPrevBills')"><i class="fas fa-xmark"></i></button>
        </div>
        <div class="pos-modal-body">
            <div class="pos-bills-toolbar">
...
<div class="pos-modal-overlay" id="modalReceipt">
    <div class="pos-modal-box">
        <div class="pos-modal-header">
            <h3><i class="fas fa-circle-check" style="color:var(--success);"></i> Sale Completed</h3>
            <button onclick="closePosModalAndReload()"><i class="fas fa-xmark"></i></button>
        </div>
        <div class="pos-modal-body" style="text-align:center;">
            <div style="background:var(--success-bg);border-radius:14px;padding:24px;margin-bottom:16px;">
                <i class="fas fa-circle-check" style="font-size:46px;color:var(--success);"></i>
...
/* =========================================================
   STATE
   ========================================================= */
var cart = [];                  // {productId,name,code,price,origPrice,qty,discPct,stock}
...
function renderProductGrid(products){
    ...
    if (!products || products.length === 0) {
        grid.html('<div style="grid-column:1/-1;text-align:center;padding:60px 0;color:var(--muted2);">' +
            '<i class="fas fa-box-open" style="font-size:36px;display:block;margin-bottom:10px;color:var(--border);"></i>No products found</div>');
        return;
    }
...
$('#btnConfirmAddToCart').on('click', function(){
    ...
    if (qty > pendingProduct.stock) { posToast('fas fa-exclamation-triangle', '', 'Only ' + pendingProduct.stock + ' available in stock', 'danger'); return; }

    // Live re-check against server stock right before adding (covers race conditions with other terminals)
...
function renderCart(){
    ...
    if (cart.length === 0) {
        list.html('<div class="pos-cart-empty" id="cartEmptyState"><i class="fas fa-cart-shopping"></i><p><b>Cart is empty</b></p><span>Scan a barcode or tap a product to begin</span></div>');
    ...
    cart.forEach(function(item, idx){
        ...
        html += '<div class="pos-citem ' + (edited ? 'edited' : '') + '" data-idx="' + idx + '">' +
            '<div class="pos-citem-top"><div><div class="pos-citem-name">' + escapeHtml(item.name) +
            (edited ? ' <span class="pos-disc-tag">Price Edited</span>' : '') + '</div>' +
            '<div class="pos-citem-code">' + escapeHtml(item.code) + '</div></div>' +
            '<button class="pos-citem-remove" data-idx="' + idx + '"><i class="fas fa-trash-can"></i></button></div>' +
...
// Mirrors cart[] into the hidden #carttable so createinvoice() can read it exactly like the old version did.
...
function renderCustomerList(term){
    $.post(BASE_URL + 'Directsale/Getcustomersearch', { searchTerm: term || '' }, function(result){
        ...
        list.forEach(function(c){
            html += '<div class="pos-cust-row" data-id="' + c.id + '" data-name="' + escapeHtml(c.name) + '" data-mobile="' + escapeHtml(c.mobile) + '" data-code="' + escapeHtml(c.code) + '">' +
        ...
        $('.pos-cust-row').on('click', function(){
            ...
            if (creditFlowPending) {
                creditFlowPending = false;
                openCreditConfirm();
            } else {
                posToast('fas fa-user-check', '', c.name + ' selected', 'success');
            }
        });
    });
}
...
$('#btnSaveNewCustomer').on('click', function(){
    ...
    if (!name || !mobile) {
        $('#newCustError').text('Name and mobile number are required.').show();
        return;
    }
    ...
    $.post(BASE_URL + 'Directsale/Quickaddcustomer', { name: name, contact: mobile }, function(result){
        var obj = JSON.parse(result);
        if (!obj.success) {
            $('#newCustError').text(obj.message || 'Could not save customer.').show();
            return;
        }
...
$('#btnCompleteCard').on('click', function(){
    ...
    // col_2 (bank) holds card type, col_4 (chequeno) holds last 4 digits — re-using existing columns per schema decision
...
$('#btnPayCredit').on('click', function(){
    if (selectedCustomer.isCash) {
        openPosModal('modalCreditBlocked');
    } else {
        openCreditConfirm();
    }
});
...
/* =========================================================
   SUBMIT SALE (shared by cash/card/credit)
   ========================================================= */
...
function loadBrowseTable(){
    $.post(BASE_URL + 'Directsale/Getproductlist', {
        ...
        products.forEach(function(p){
            var s = stockInfo(p.stock);
            html += '<tr data-id="' + p.id + '" data-name="' + escapeHtml(p.name) + '" data-code="' + escapeHtml(p.code) + '" data-price="' + p.price + '" data-stock="' + p.stock + '" style="cursor:pointer;">' +
...
$('#btnPrevBills').on('click', function(){
    ...
    prevBillsTable = $('#tablePrevBills').DataTable({
        ...
        columns: [
            ...
            { data: 'invtype', render: function(d, type, row){
                var label = (d == 1) ? 'Cash' : (d == 2 ? 'Card' : 'Credit');
                var cls = (d == 3) ? 'credit' : 'paid';
            ...
            { data: 'idtbl_invoice', orderable: false, render: function(d, type, row){
                var printUrl = (row.invtype == 3)
                    ? BASE_URL + 'Directsale/Getcreditprintbill/' + d
                    : BASE_URL + 'Directsale/Getposprintbill/' + d;
                return '<div class="pos-row-actions"><button onclick="window.open(\'' + printUrl + '\',\'_blank\')" title="Print"><i class="fas fa-print"></i></button></div>';
            }}
        ]
    });
}
