...
<div class="pos-modal-overlay" id="modalAdvanceOrder">
    <div class="pos-modal-box">
        ...
        <div class="pos-modal-body">
            ...
            <div id="advError" style="color:var(--danger);font-size:12px;display:none;"></div>
        </div>
        <div class="pos-modal-footer">
            <button class="pos-btn pos-btn-secondary" onclick="closePosModal('modalAdvanceOrder')">Cancel</button>
            <button class="pos-btn pos-btn-success" id="btnCompleteAdvance"><i class="fas fa-check"></i> Save Advance Order</button>
        </div>
    </div>
</div>
    ...
    <div class="pos-modal-box xwide">
        ...
        <div class="pos-modal-body">
            ...
            <table class="table table-bordered table-striped table-sm">
                <thead><tr><th>Invoice</th><th>Customer</th><th>Due Date</th><th class="text-right">Total</th><th class="text-right">Paid</th><th class="text-right">Balance</th><th></th></tr></thead>
...
function updateSummary(){
    ...
    $('#btnPayCash, #btnPayCard, #btnPayCredit, #btnAddPayment').prop('disabled', !hasItems);
...
$('#btnPayAdvance').on('click', function(){
    if (selectedCustomer.isCash) { posToast('fas fa-exclamation-triangle','','Advance orders require a saved customer','danger'); openPosModal('modalCustomerSelect'); renderCustomerList(''); return; }
    var s = updateSummary();
...
/* submitSale extended to accept extra POST fields */
function submitSale(billtype, extra){
    // ...existing jsonObj / jsonObjPay building unchanged...
    var s = updateSummary();
    var paytotal = 0;
    jsonObjPay.forEach(function(p){ paytotal += parseFloat(p.col_6) || 0; });

    var payload = {
        tableData: jsonObj, tableDataPay: jsonObjPay,
        total: s.gross, distotal: s.totalDiscount, nettotal: s.net, paytotal: paytotal,
        billtype: billtype, customer: selectedCustomer.id,
    ...
    $.ajax({
        type: 'POST', url: BASE_URL + 'Directsale/Directsaleinsertupdate', data: payload,
        success: function(result){
            var obj = JSON.parse(result);
            if (obj.actiontype == '1') {
                $('#receiptInvNo').text('INV-' + String(obj.invoiceid).padStart(6,'0'));
                $('#receiptMethod').text(billtype==1?'Cash':billtype==2?'Card':billtype==4?'Advance Order':'Credit');
                openPosModal('modalReceipt');
                var url = (billtype == 3 || billtype == 4)
                    ? BASE_URL + 'Directsale/Getcreditprintbill/' + obj.invoiceid
                    : BASE_URL + 'Directsale/Getposprintbill/' + obj.invoiceid;
                window.open(url, '_blank');
            } else {
                var a = JSON.parse(obj.action);
                posToast(a.icon, a.title, a.message, a.type);
            }
        },
...
$('#btnCompleteSettle').on('click', function(){
    ...
    $.post(BASE_URL + 'Directsale/Settleadvancebalance', {
        invoiceID: invoiceID, amount: amount, method: $('#settleMethod').val(),
        ...
        posToast('fas fa-check-circle','', obj.settled ? 'Order fully settled' : 'Partial payment recorded', 'success');
