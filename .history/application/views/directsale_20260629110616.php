...
<div class="pos-modal-overlay" id="modalBatchPicker">
    <div class="pos-modal-box">
        ...
        <div class="pos-modal-body">
            ...
            <div id="batchListBody" style="display:flex;flex-direction:column;gap:8px;max-height:340px;overflow-y:auto;"></div>
...
/* =========================================================
   ...
   ========================================================= */
/* =========================================================
   BATCH PICKER → then ADD TO CART
   ========================================================= */
function openBatchPicker(product) {
    pendingProduct = product;
    $('#batchProductName').text(product.name);
    $('#batchProductCode').text(product.code);
    $('#batchListBody').html('<div style="text-align:center;padding:20px;color:var(--muted2);"><i class="fas fa-spinner fa-spin"></i> Loading batches…</div>');
    $('#batchNoneMsg').hide();
    ...
    $.post(BASE_URL + 'Directsale/Getproductbatches', { productID: product.id }, function(result){
        ...
        batches.forEach(function(b, idx){
            ...
            html += '<div class="pos-batch-row" data-idx="' + idx + '" style="' +
                'display:flex;align-items:center;justify-content:space-between;' +
                'border:1px solid var(--border);border-radius:10px;padding:10px 14px;cursor:pointer;' +
                'transition:background 0.15s;background:var(--surface-2);"' +
                ' onmouseover="this.style.background=\'var(--accent-light)\'"' +
                ' onmouseout="this.style.background=\'var(--surface-2)\'">' +
                '<div>' +
                  '<div style="font-weight:700;font-size:13.5px;color:var(--primary-dark);">' +
                    '<i class="fas fa-tag" style="margin-right:5px;font-size:11px;"></i>' +
                    'Batch: ' + escapeHtml(b.batchno || 'N/A') +
                  '</div>' +
                  '<div style="font-size:11.5px;color:var(--muted2);margin-top:3px;">' +
                    'Received: ' + (dateStr || '—') +
                  '</div>' +
                '</div>' +
                '<div style="text-align:right;">' +
                  '<div style="font-size:15px;font-weight:700;color:var(--success);">' + money(b.saleprice) + '</div>' +
                  '<div style="font-size:11.5px;color:var(--muted2);margin-top:2px;">' +
                    '<i class="fas fa-boxes" style="margin-right:3px;"></i>Qty: ' + b.qty +
                  '</div>' +
                '</div>' +
                '</div>';
        });
        ...
        // Store batches on the element for click handler
        $('#batchListBody').data('batches', batches);

        $('.pos-batch-row').on('click', function(){
            var idx = $(this).data('idx');
            var b = $('#batchListBody').data('batches')[idx];
            closePosModal('modalBatchPicker');
            // Open the existing add-to-cart modal pre-filled with this batch's price
            openAddToCart({
...
function renderCart(){
    ...
    cart.forEach(function(item, idx){
        ...
        html += '<div class="pos-citem ' + (edited ? 'edited' : '') + '" data-idx="' + idx + '">' +
            ...
            '<button class="pos-citem-remove" data-idx="' + idx + '"><i class="fas fa-trash-alt"></i></button></div>' +
    ...
    $.post(BASE_URL + 'Directsale/Getcustomersearch', { searchTerm: term || '' }, function(result){
        ...
        $('.pos-cust-row').on('click', function(){
            ...
            if (creditFlowPending) {
                ...
                posToast('fas fa-check-circle', '', c.name + ' selected', 'success');
            }
        });
    });
}
...
/* =========================================================
   ...
   These buttons are wired to the new top bar per the redesign,
   but hold/return workflows are not yet implemented server-side.
   Showing an honest "coming soon" toast rather than a dead button.
   ========================================================= */
$('#btnHeldBills').on('click', function(){
    posToast('fas fa-info-circle', '', 'Held bills feature is coming soon.', 'info');
});
$('#btnReturns').on('click', function(){
    posToast('fas fa-info-circle', '', 'Returns feature is coming soon.', 'info');
});
