                ...
                <div class="row mt-2">
                    <div class="col-md-4 mb-3">
                        <div class="panel-card">
                            <div class="panel-title"><i class="fas fa-calendar-alt text-info"></i> Calendar</div>
                            <form id="monthForm" method="get" style="margin:0 0 12px 0;">
                                <input type="hidden" name="month" id="monthPicker" value="<?php echo isset($selectedMonth)?htmlspecialchars($selectedMonth):''; ?>">
                            </form>
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    // If a native month picker exists, still submit on change
    if (mp && mp.tagName.toLowerCase() === 'input' && mp.type === 'month') mp.addEventListener('change', function(){ document.getElementById('monthForm').submit(); });

    // Initialize inline bootstrap datepicker for dashboard calendar
    if (typeof $ !== 'undefined' && typeof $.fn.datepicker === 'function') {
        $('#dashboardCalendar').datepicker({
            todayHighlight: true,
            format: 'yyyy-mm-dd',
            autoclose: false
        ...
        // Pre-select if a month was provided: select first day of that month
        <?php if (!empty($selectedMonth)): ?>
            (function(){
                var parts = '<?php echo $selectedMonth; ?>'.split('-');
                if (parts.length===2) {
                    var sd = new Date(parts[0], parseInt(parts[1],10)-1, 1);
                    $('#dashboardCalendar').datepicker('update', sd);
                }
            })();
        <?php endif; ?>
    }
