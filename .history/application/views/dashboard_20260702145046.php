...
/* Date filter badge (calendar trigger) */
.date-filter-badge {
    display: inline-flex;
    align-items: center;
    background: rgba(255,255,255,.15);
...
.date-filter-badge:hover { background: rgba(255,255,255,.25); }
.date-filter-badge .cal-icon {
    background: rgba(255,255,255,.25);
    width: 36px; height: 36px;
    border-radius: 50%;
    display:flex; align-items:center; justify-content:center;
...
.date-filter-badge .chev { margin-left: .65rem; opacity:.8; font-size:.75rem; }
.date-filter-today {
    margin-left: .5rem;
    background: rgba(255,255,255,.15);
    border: 2px solid rgba(255,255,255,.3);
...
.date-filter-today:hover { background: rgba(255,255,255,.25); color:#fff; text-decoration:none; }
.viewing-date-note {
    font-size: .8rem;
    color: #8a8f9a;
    margin-top: -0.5rem;
...
/* Flatpickr theme tweaks to match teal branding */
    ...
    <div id="layoutSidenav_content">
        <main>
            <div class="page-header text-white">
                <div class="container-fluid">
                    <div class="page-header-content py-3 d-flex align-items-center justify-content-between flex-wrap">
                        <h1 class="page-header-title d-flex align-items-center mb-2 mb-md-0">
                            <div class="page-header-icon mr-2"><i class="fas fa-store-alt"></i></div>
                            <span>Dashboard</span>
                        </h1>
                        <div class="d-flex align-items-center flex-wrap">
                            <div class="date-filter-badge" id="dateFilterTrigger">
                                <div class="cal-icon"><i class="fas fa-calendar-alt"></i></div>
                                <span id="selectedDateLabel"><?php echo date('F j, Y', strtotime($selectedDate)); ?></span>
                                <i class="fas fa-chevron-down chev"></i>
                            </div>
                            <input type="text" id="dashboardDatePicker" style="display:none;">
                            <?php if ($selectedDate !== date('Y-m-d')): ?>
                                <a href="javascript:void(0);" class="date-filter-today" onclick="goToDate('<?php echo date('Y-m-d'); ?>');">
                                    <i class="fas fa-undo mr-1"></i> Today
                                </a>
                            <?php endif; ?>
                            <div class="welcome-company-badge ml-3">
            ...
            <div class="container-fluid mt-4 ml-2">
                ...
                <?php if ($selectedDate !== date('Y-m-d')): ?>
                    <div class="viewing-date-note">
                        <i class="fas fa-info-circle mr-1"></i>
                        Viewing dashboard data for <strong><?php echo date('F j, Y', strtotime($selectedDate)); ?></strong>
                    </div>
                <?php endif; ?>
...
document.addEventListener("DOMContentLoaded", function() {
    var picker = flatpickr("#dashboardDatePicker", {
    ...
    document.getElementById('dateFilterTrigger').addEventListener('click', function() {
        picker.open();
    });
