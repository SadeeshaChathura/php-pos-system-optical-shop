            ...
            <div class="container-fluid p-0 pos-container pos-fullscreen">
                ...
                <div class="pos-exit-bar">
                    ...
                    <div class="pos-exit-bar-mid">
                        ...
                        <button class="pos-topbar-btn" id="btnPendingOrders"><i class="fas fa-hourglass-half"></i> Pending Orders</button>
                    ...
                    <div class="pos-right">
                        ...
                        <div class="pos-cart-summary">
                            ...
                            <div class="pos-pay-buttons">
                                <button class="pos-pay-btn cash h-100" id="btnPayCash" disabled><i class="fas fa-money-bill-wave"></i> Cash</button>
                                <button class="pos-pay-btn card h-100" id="btnPayCard" disabled><i class="fas fa-credit-card"></i> Card</button>
                                <button class="pos-pay-btn advance h-100" id="btnPayAdvance" disabled><i class="fas fa-clock"></i> Advance Order</button>
                                <button class="pos-pay-btn credit h-100" id="btnPayCredit" disabled><i class="fas fa-file-invoice"></i> Sell on Credit</button>
                            </div>
                ...
                <table class="table d-none" id="carttable">
                    <thead>
                        <tr>
                            ...
                            <th>productid</th><th>productcode</th><th>batchno</th>
                        </tr>
                    </thead>
                    <tbody></tbody>
                </table>
...
<!-- ===================== MODAL: ADVANCE / PRODUCTION ORDER ===================== -->
<div class="pos-modal-overlay" id="modalAdvanceOrder">
    <div class="pos-modal-box">
        <div class="pos-modal-header">
            <h3><i class="fas fa-clock"></i> Advance / Production Order</h3>
            <button onclick="closePosModal('modalAdvanceOrder')"><i class="fas fa-times"></i></button>
        </div>
        <div class="pos-modal-body">
            <div class="pos-cust-row" style="display:flex;align-items:center;gap:10px;border:1px solid var(--border);border-radius:10px;padding:10px;margin-bottom:14px;">
                <div class="av" style="width:34px;height:34px;border-radius:50%;background:var(--primary);color:#fff;display:flex;align-items:center;justify-content:center;font-weight:700;font-size:12px;">--</div>
                <div><b id="advCustName">—</b><br><span style="font-size:11.5px;color:var(--muted2);" id="advCustMeta">—</span></div>
            </div>
            <div class="pos-bill-summary-box">
                <div class="lbl">Order Total</div>
                <div class="amt" id="advOrderTotal">Rs. 0.00</div>
            </div>
            <div class="pos-form-group">
                <label>Due Date (pickup / ready date) *</label>
                <input type="date" class="pos-form-control" id="advDueDate">
            </div>
            <div class="pos-form-row">
                <div class="pos-form-group">
                    <label>Advance Amount Received *</label>
                    <input type="number" class="pos-form-control" id="advAmount" min="1">
                </div>
                <div class="pos-form-group">
                    <label>Method</label>
                    <select class="pos-form-control" id="advMethod">
                        <option value="Cash">Cash</option>
                        <option value="Card">Card</option>
                    </select>
                </div>
            </div>
            <div class="pos-form-row" id="advCardRow" style="display:none;">
                <div class="pos-form-group" style="flex:1.4;">
                    <label>Card Type</label>
                    <select class="pos-form-control" id="advCardType"><option>Visa</option><option>MasterCard</option><option>Amex</option><option>Other</option></select>
                </div>
                <div class="pos-form-group">
                    <label>Last 4 Digits</label>
                    <input type="text" class="pos-form-control" id="advCardLast4" maxlength="4">
                </div>
            </div>
            <div style="display:flex;align-items:center;gap:8px;margin-top:6px;padding:10px 12px;background:var(--accent-light);border-radius:9px;">
                <input type="checkbox" id="advWarrantyCheck" style="width:16px;height:16px;cursor:pointer;">
                <label for="advWarrantyCheck" style="margin:0;font-size:12.5px;font-weight:600;color:var(--primary-dark);cursor:pointer;">
                    Add Lifetime Warranty to this invoice
                </label>
            </div>
            <div id="advError" style="color:var(--danger);font-size:12px;display:none;margin-top:10px;"></div>
        </div>
        <div class="pos-modal-footer">
            <button class="pos-btn pos-btn-secondary" onclick="closePosModal('modalAdvanceOrder')">Cancel</button>
            <button class="pos-btn pos-btn-success" id="btnCompleteAdvance"><i class="fas fa-check"></i> Save &amp; Print Advance Bill</button>
        </div>
    </div>
</div>

<!-- ===================== MODAL: PENDING ADVANCE ORDERS ===================== -->
<div class="pos-modal-overlay" id="modalPendingOrders">
    <div class="pos-modal-box xwide">
        <div class="pos-modal-header">
            <h3><i class="fas fa-hourglass-half"></i> Pending Advance Orders</h3>
            <button onclick="closePosModal('modalPendingOrders')"><i class="fas fa-times"></i></button>
        </div>
        <div class="pos-modal-body">
            <div class="pos-bills-toolbar">
                <input type="text" class="pos-form-control" id="pendingSearch" placeholder="Search customer name, mobile, or invoice #…">
            </div>
            <table class="table table-bordered table-striped table-sm">
                <thead>
                    <tr><th>Invoice</th><th>Customer</th><th>Due Date</th><th class="text-right">Total</th><th class="text-right">Paid</th><th class="text-right">Balance</th><th></th></tr>
                </thead>
...
<!-- ===================== MODAL: SETTLE BALANCE ===================== -->
<div class="pos-modal-overlay" id="modalSettleBalance">
    <div class="pos-modal-box">
        <div class="pos-modal-header">
            <h3><i class="fas fa-hand-holding-usd"></i> Collect Balance</h3>
            <button onclick="closePosModal('modalSettleBalance')"><i class="fas fa-times"></i></button>
        </div>
        <div class="pos-modal-body">
            <div class="pos-bill-summary-box">
                <div class="lbl">Balance Due — <span id="settleCustName"></span></div>
                <div class="amt" id="settleBalanceAmt">Rs. 0.00</div>
            </div>
            <input type="hidden" id="settleInvoiceID">
            <div class="pos-form-row">
                <div class="pos-form-group">
                    <label>Amount Received</label>
                    <input type="number" class="pos-form-control" id="settleAmount">
                </div>
                <div class="pos-form-group">
                    <label>Method</label>
                    <select class="pos-form-control" id="settleMethod">
                        <option value="Cash">Cash</option>
                        <option value="Card">Card</option>
                    </select>
                </div>
            </div>
            <div class="pos-form-row" id="settleCardRow" style="display:none;">
                <div class="pos-form-group" style="flex:1.4;">
                    <label>Card Type</label>
                    <select class="pos-form-control" id="settleCardType"><option>Visa</option><option>MasterCard</option><option>Amex</option><option>Other</option></select>
                </div>
                <div class="pos-form-group">
                    <label>Last 4 Digits</label>
                    <input type="text" class="pos-form-control" id="settleCardLast4" maxlength="4">
                </div>
            </div>
        </div>
        <div class="pos-modal-footer">
            <button class="pos-btn pos-btn-secondary" onclick="closePosModal('modalSettleBalance')">Cancel</button>
            <button class="pos-btn pos-btn-success" id="btnCompleteSettle"><i class="fas fa-check"></i> Complete Payment &amp; Print</button>
        </div>
    </div>
</div>
...
<div class="pos-modal-overlay" id="modalPrevBills">
    <div class="pos-modal-box xwide">
        ...
        <div class="pos-modal-body">
            <div class="pos-bills-toolbar">
                <input type="date" class="pos-form-control" id="prevBillsDateFrom" style="max-width:170px;">
                <input type="date" class="pos-form-control" id="prevBillsDateTo" style="max-width:170px;">
                <select class="pos-form-control" id="prevBillsType" style="max-width:170px;">
            ...
            <table class="table table-bordered table-striped table-sm" id="tablePrevBills" style="width:100%;">
                <thead>
                    <tr>
                        <th>Invoice</th>
                        <th>Date</th>
        ...
        <div class="pos-modal-footer">
            <button class="pos-btn pos-btn-secondary" onclick="closePosModal('modalPrevBills')">Close</button>
        </div>
    </div>
</div>
...
function openPosModal(id){ document.getElementById(id).classList.add('show'); }A
...
function updateSummary(){
    ...
    $('#btnPayCash, #btnPayCard, #btnPayCredit, #btnPayAdvance').prop('disabled', !hasItems);
...
function renderCustomerList(term){
    $.post(BASE_URL + 'Directsale/Getcustomersearch', { searchTerm: term || '' }, function(result){
        ...
        $('.pos-cust-row').on('click', function(){
            ...
            else if (advanceFlowPending) { advanceFlowPending = false; openAdvanceOrderModal(); }
...
/* =========================================================
   PAYMENT: ADVANCE / PRODUCTION ORDER
   ========================================================= */
$('#btnPayAdvance').on('click', function(){
    if (selectedCustomer.isCash) {
        posToast('fas fa-exclamation-triangle', '', 'Advance orders require a saved customer', 'danger');
        advanceFlowPending = true;
        renderCustomerList('');
        openPosModal('modalCustomerSelect');
        return;
    }
...
function openAdvanceOrderModal(){
    var s = updateSummary();
    $('#advCustName').text(selectedCustomer.name);
    $('#advCustMeta').text((selectedCustomer.mobile || '') + ' · ' + selectedCustomer.code);
    $('#advOrderTotal').text(money(s.net));
    $('#advAmount').val(s.net.toFixed(2));

    var d = new Date(); d.setDate(d.getDate() + 6);
    $('#advDueDate').val(d.toISOString().slice(0,10));

    $('#advMethod').val('Cash');
    $('#advCardRow').hide();
    $('#advCardLast4').val('');
    $('#advWarrantyCheck').prop('checked', false);
    $('#advError').hide();
    openPosModal('modalAdvanceOrder');
}

$('#advMethod').on('change', function(){ $('#advCardRow').toggle($(this).val() === 'Card'); });

$('#btnCompleteAdvance').on('click', function(){
    var due = $('#advDueDate').val();
    var amt = parseFloat($('#advAmount').val()) || 0;
    var net = parseMoney($('#advOrderTotal').text());
    var method = $('#advMethod').val();

    if (!due) { $('#advError').text('Please select a due date.').show(); return; }
    if (amt <= 0 || amt > net) { $('#advError').text('Enter a valid advance amount (up to order total).').show(); return; }
    if (method === 'Card' && $('#advCardLast4').val().length !== 4) {
        $('#advError').text('Enter the last 4 digits of the card.').show();
        return;
    }
    $('#advError').hide();

    $('#hidewarrantystatus').val($('#advWarrantyCheck').is(':checked') ? '1' : '0');
    $('#paymenttable tbody').empty().append(
        '<tr><td>' + method + '</td><td>' + $('#advCardType').val() + '</td><td></td><td>' + $('#advCardLast4').val() + '</td><td></td><td>' + amt.toFixed(2) + '</td></tr>'
    );
    closePosModal('modalAdvanceOrder');
    submitSale(4, { ordertype: 2, duedate: due });
});

/* =========================================================
   SUBMIT SALE (single source of truth)
   billtype: 1 Cash, 2 Card, 3 Credit, 4 Advance/Production Order
   extra: optional object merged into the POST payload
          (used to send ordertype + duedate for advance orders)
   ========================================================= */
function submitSale(billtype, extra){
    var jsonObj = [];
    ...
    $.ajax({
        ...
        success: function(result){
            ...
            if (obj.actiontype == '1') {
                ...
                var methodLabel = billtype == 1 ? 'Cash' : billtype == 2 ? 'Card' : billtype == 4 ? 'Advance Order' : 'Credit';
...
/* =========================================================
   PENDING ORDERS SEARCH & SETTLE
   ========================================================= */
$('#btnPendingOrders').on('click', function(){ openPosModal('modalPendingOrders'); loadPendingOrders(''); });
$('#pendingSearch').on('input', debounce(function(){ loadPendingOrders($(this).val()); }, 300));

function loadPendingOrders(term){
    $.post(BASE_URL + 'Directsale/Getpendingadvanceorders', { searchTerm: term }, function(result){
        var list;
        try { list = JSON.parse(result); } catch(e){ list = []; }
        var body = $('#pendingOrdersBody');
        if (!list.length) { body.html('<tr><td colspan="7" class="text-center">No pending orders</td></tr>'); return; }
        var html = '';
        list.forEach(function(o){
            html += '<tr' + (o.overdue ? ' class="reorder-alert"' : '') + '>' +
                '<td>' + o.invno + '</td>' +
                '<td>' + escapeHtml(o.customer) + '<br><small class="text-muted">' + escapeHtml(o.contact||'') + '</small></td>' +
                '<td>' + (o.duedate || '—') + (o.overdue ? ' <span class="text-danger font-weight-bold">(overdue)</span>' : '') + '</td>' +
                '<td class="text-right">' + money(o.nettotal) + '</td>' +
                '<td class="text-right">' + money(o.paidsofar) + '</td>' +
                '<td class="text-right font-weight-bold">' + money(o.balance) + '</td>' +
                '<td><button class="btn btn-sm btn-success btn-settle" data-id="' + o.invoiceid + '">Collect</button></td>' +
                '</tr>';
        });
        body.html(html);
        $('.btn-settle').on('click', function(){ openSettleModal($(this).data('id')); });
    });
}

function openSettleModal(invoiceID){
    $.post(BASE_URL + 'Directsale/Getadvanceorderbalance', { invoiceID: invoiceID }, function(result){
        var o = JSON.parse(result);
        if (!o.found) { posToast('fas fa-exclamation-triangle','','Order not found','danger'); return; }
        $('#settleInvoiceID').val(o.invoiceid);
        $('#settleCustName').text(o.customer);
        $('#settleBalanceAmt').text(money(o.balance));
        $('#settleAmount').val(o.balance.toFixed(2));
        $('#settleCardRow').hide();
        $('#settleMethod').val('Cash');
        $('#settleCardLast4').val('');
        closePosModal('modalPendingOrders');
        openPosModal('modalSettleBalance');
    });
}
$('#settleMethod').on('change', function(){ $('#settleCardRow').toggle($(this).val() === 'Card'); });

$('#btnCompleteSettle').on('click', function(){
    var invoiceID = $('#settleInvoiceID').val();
    var amount    = parseFloat($('#settleAmount').val()) || 0;
    var method    = $('#settleMethod').val();

    if (amount <= 0) { posToast('fas fa-exclamation-triangle','','Enter a valid amount','danger'); return; }
    if (method === 'Card' && $('#settleCardLast4').val().length !== 4) {
        posToast('fas fa-exclamation-triangle','','Enter the last 4 digits of the card','danger'); return;
    }

    $.post(BASE_URL + 'Directsale/Settleadvancebalance', {
        invoiceID: invoiceID, amount: amount, method: method,
        cardType: $('#settleCardType').val(), cardLast4: $('#settleCardLast4').val()
    }, function(result){
        var obj;
        try { obj = JSON.parse(result); } catch(e){ posToast('fas fa-exclamation-triangle','','Unexpected server response','danger'); return; }
        if (!obj.success) { posToast('fas fa-exclamation-triangle','',obj.message||'Could not save payment','danger'); return; }
        ...
        if (obj.settled) {
            setTimeout(function(){ loadPendingOrders($('#pendingSearch').val()); }, 800);
        } else {
            loadPendingOrders($('#pendingSearch').val());
        }
    });
});
