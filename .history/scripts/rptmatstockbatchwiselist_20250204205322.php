...
$joinQuery = "FROM (SELECT u.idtbl_stock AS idtbl_stock, SUM(u.qty) AS total_qty,ua.materialinfocode AS materialinfocode,ua.materialname AS materialname,u.status AS status FROM tbl_stock AS u JOIN tbl_material_info AS ua ON (ua.idtbl_material_info = u.tbl_material_info_idtbl_material_info) JOIN tbl_unit AS ub ON (ub.idtbl_unit = ua.tbl_unit_idtbl_unit) WHERE u.status IN (1, 2) AND u.qty != 0  AND `u`.`tbl_company_idtbl_company`='$companyid' AND `u`.`tbl_company_branch_idtbl_company_branch`='$branchid' GROUP BY ua.materialinfocode) AS main";
