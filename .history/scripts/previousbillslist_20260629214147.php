...
$columns = array(
    ...
    array( 'db' => '`ua`.`contact`',       'dt' => 'contact','field' => 'contact' ),
...
LEFT JOIN `tbl_invoice_payment_has_tbl_invoice` AS `iph`
    ON `iph`.`tbl_invoice_idtbl_invoice` = `u`.`idtbl_invoice`
LEFT JOIN `tbl_invoice_payment` AS `p`
    ON `p`.`idtbl_invoice_payment` = `iph`.`tbl_invoice_payment_idtbl_invoice_payment`
...
if (!empty($_POST['filterInvType'])) {
    ...
    if ($invType == 3) {
        // CREDIT = invoices WITHOUT payment method (NULL or missing)
        $extraWhere .= " AND (`pd`.`method` IS NULL OR `pd`.`method` = 0)";
    } else {
        // CASH (1) or CARD (2)
        $extraWhere .= " AND `pd`.`method` = " . $invType;
    }
}
