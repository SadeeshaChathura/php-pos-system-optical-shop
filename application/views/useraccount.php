...
<script>
    $(document).ready(function() {
        ...
        $('#dataTable').DataTable({
            ...
            buttons: [
                { extend: 'csv', className: 'btn btn-ocean-blue btn-sm', title: 'User Account Information', text: '<i class="fas fa-file-csv mr-2"></i> CSV' },
                { extend: 'pdf', className: 'btn btn-ocean-blue btn-sm', title: 'User Account Information', text: '<i class="fas fa-file-pdf mr-2"></i> PDF' },
                { 
                    extend: 'print', 
            ...
            "columns": [
                ...
                },
                {
                    ...
                    "render": function(data, type, full) {
                        var button='';
                        button+='<button class="btn btn-primary btn-sm btnEdit mr-1 ';if(editcheck!=1){button+='d-none';}button+='" id="'+full['idtbl_user']+'" data-toggle="tooltip" title="Edit"><i class="fas fa-edit"></i></button>';
                        if(full['status']==1){
                            button+='<a href="<?php echo base_url() ?>User/Useraccountstatus/'+full['idtbl_user']+'/2" onclick="return deactive_confirm()" target="_self" class="btn btn-success btn-sm mr-1 ';if(statuscheck!=1){button+='d-none';}button+='" data-toggle="tooltip" title="Deactivate"><i class="fas fa-check-square"></i></a>';
                        }else{
                            button+='<a href="<?php echo base_url() ?>User/Useraccountstatus/'+full['idtbl_user']+'/1" onclick="return active_confirm()" target="_self" class="btn btn-warning btn-sm mr-1 ';if(statuscheck!=1){button+='d-none';}button+='" data-toggle="tooltip" title="Activate"><i class="fas fa-times-circle"></i></a>';
                        }
                        button+='<a href="<?php echo base_url() ?>User/Useraccountstatus/'+full['idtbl_user']+'/3" onclick="return delete_confirm()" target="_self" class="btn btn-danger btn-sm ';if(deletecheck!=1){button+='d-none';}button+='" data-toggle="tooltip" title="Delete"><i class="fas fa-trash-alt"></i></a>';
    ...
    function resetForm() {
        ...
        $('#accountname').val('');
        $('#username').val('');
        $('#password').val('').attr('required', 'required');
        $('#usertype').val('');
