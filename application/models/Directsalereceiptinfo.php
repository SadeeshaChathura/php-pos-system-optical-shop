...
class Directsalereceiptinfo extends CI_Model {
   ...
   public function Getposprintbill($x){
        ...
        $sqlinvoiceinfo = "SELECT `tbl_invoice`.`idtbl_invoice`, `tbl_invoice`.`invdate`, `tbl_invoice`.`grosstotal`,
                                ...
                                `tbl_invoice`.`paymentgiven`, `tbl_invoice`.`changegiven`,
                                `tbl_customer`.`name` AS customername, 
                                `tbl_customer`.`contact` AS customercontact,
                                `tbl_customer`.`customercode` AS customercode
                            FROM `tbl_invoice`
                            LEFT JOIN `tbl_customer` 
                                ON `tbl_customer`.`idtbl_customer` = `tbl_invoice`.`tbl_customer_idtbl_customer`
                            WHERE `tbl_invoice`.`status`=1 
                            AND `tbl_invoice`.`idtbl_invoice`=?";
        ...
        $invtype        = $row->invtype;   // 1 = POS (Cash/Card), 2 = Credit
        $warrantystatus = $row->warrantystatus;
        $paymentgiven   = $row->paymentgiven;
        $changegiven    = $row->changegiven;
        $custname       = $row->customername;
        $custcontact    = $row->customercontact;
        ...
        $sqlproduct = "SELECT `tbl_material_info`.`materialinfocode`, `tbl_material_info`.`materialname`,
                            ...
                            `tbl_invoice_detail`.`discount` AS linediscount
                        ...
                        WHERE `tbl_invoice_detail`.`tbl_invoice_idtbl_invoice`=? 
        ...
        $sqlpay = "SELECT `method`, `bank`, `chequeno`
                   FROM `tbl_invoice_payment_detail` pd
                   LEFT JOIN `tbl_invoice_payment_has_tbl_invoice` ph
                   ON ph.tbl_invoice_payment_idtbl_invoice_payment = pd.tbl_invoice_payment_idtbl_invoice_payment
                   WHERE ph.tbl_invoice_idtbl_invoice=?
                   LIMIT 1";

        $payRespond = $this->db->query($sqlpay, array($recordID));

        if ($payRespond->num_rows() > 0) {
            $paymentMethod = (int)$payRespond->row(0)->method;

            if ($paymentMethod == 2) {
                $cardTypeLabel = $payRespond->row(0)->bank;
                $cardLast4     = $payRespond->row(0)->chequeno;
            }
        }
        ...
        foreach ($productrespond->result() as $rowlist) {
            ...
            $tblinvoice .= '
                <tr>
                    ...
                    <td>
                        <div class="pname">'.htmlspecialchars($rowlist->materialname).'</div>
                        <div class="pcode">'.htmlspecialchars($rowlist->materialinfocode).'</div>
                    </td>
        ...
        /* =========================
           LABELS FIXED
        ========================== */
        $invTypeLabel = ($invtype == 2)
            ? 'Credit Sale'
            : (($paymentMethod == 2) ? 'Card Sale' : 'Cash Sale');

        $statusPillClass = ($invtype == 2) ? 'credit' : '';
        $statusPillText  = ($invtype == 2) ? '&#8987; CREDIT' : '&#10003; PAID';
        ...
        if ($invtype == 2) {
            ...
            $payStripHtml = '
                <div class="pay-strip">
                    <span class="pay-badge">CREDIT</span>
            ...
            if ($paymentMethod == 2) {
                $payStripHtml = '
                    <div class="pay-strip">
                        ...
                        '.htmlspecialchars($cardTypeLabel).' '.($cardLast4 ? ' ending **** '.htmlspecialchars($cardLast4) : '').'
                    </div>';
            } else {
                $payStripHtml = '
                    <div class="pay-strip">
        ...
        if ($invtype == 2) {
            ...
            $extraTotalsHtml = '
                <div class="due-row"><span>Due Amount</span><span>Rs. '.number_format($nettotal,2).'</span></div>
            ';

        } else {

            $extraTotalsHtml = '
                <div class="paid-row"><span>Paid</span><span>Rs. '.number_format($paymentgiven,2).'</span></div>
                <div class="change-row"><span>Change</span><span>Rs. '.number_format($changegiven,2).'</span></div>
            ';
        }
        ...
        $html = $this->_renderInvoiceShell(array(
            ...
            'cust_name'        => $custname ?: 'Cash Customer',
            'cust_meta'        => $custcontact ? ($custcontact.' · '.($custcode ?: 'CASH')) : 'Walk-in',
            'sale_type_label'  => $invTypeLabel,
            'sale_type_sub'    => ($invtype == 2) ? 'Credit customer' : 'Walk-in sale',
            'itemcount'        => $itemcount,
            'sumqty'           => $sumqty,
        ...
        return '
            <div class="warranty-box">
                ...
                <div class="w-text">
                    ...
                    <div class="w-desc">
                        This frame is covered by CalciteX Optical Lifetime Warranty against manufacturing
